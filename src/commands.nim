...
proc captureScreenPng*(): seq[byte] =
  ...
  # Extract pixels BEFORE releasing memDC — use memDC, not a new DC
  var bmi: BITMAPINFO
  ...
  discard GetDIBits(memDC, bmp, 0, UINT(h), addr pixels[0], addr bmi, DIB_RGB_COLORS)
  ...
  let bh = cast[uint32](-h.int32)   # negative = top-down
