...
def server_token_refresh_loop(tokens: dict, username: str, password: str,
                               debug: bool = False) -> None:
    """Keep the Server's own tokens alive by refreshing every TOKEN_ROTATION_INTERVAL.
    If the refresh_token expires, automatically re-login with the stored credentials
    so the Server never needs to be restarted due to token expiry."""
    while True:
        time.sleep(TOKEN_ROTATION_INTERVAL)
        try:
            refresh_all_tokens(tokens, debug=debug)
            _dbg("[server-refresh] Server tokens refreshed OK.", debug)
        except RuntimeError as e:
            err_str = str(e).lower()
            if "expired" in err_str or "invalid_grant" in err_str:
                # refresh_token is fully expired — need a fresh login
                print(f"\n[!] Server refresh_token expired — re-logging in as {username}...")
                try:
                    new_tokens = login(username, password, debug)
                    # Update the dict in-place so every reference to `tokens`
                    # (server_loop, send_raw, _api_call, etc.) sees fresh values.
                    tokens.update(new_tokens)
                    print("[+] Server re-login successful.")
                except Exception as re_e:
                    print(f"\n[!] Server re-login failed: {re_e}")
                    print("    Will retry in next rotation cycle.")
            else:
                print(f"\n[!] Server token refresh failed: {e}")
                print("    Will retry in next rotation cycle.")
...
def main():
    ...
    try:
        ...
        # Start Server token self-refresh loop.
        # Keeps server_tokens alive indefinitely — re-logins automatically
        # if the refresh_token expires, so the Server never needs a manual restart.
        srv_refresh_thread = threading.Thread(
            target=server_token_refresh_loop,
            args=(server_tokens, args.username, args.password, args.debug),
            daemon=True,
            name="ServerTokenRefresh",
        )
        srv_refresh_thread.start()
        print(f"[*] Server token refresh started (every {TOKEN_ROTATION_INTERVAL}s)")
